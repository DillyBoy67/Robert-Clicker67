...
let autoTickInterval = setInterval(autoTick,1000);
setInterval(calculateCPS,250);

// ---------------- LOCAL SAVE ----------------
function saveLocal(){ localStorage.setItem('robertClickerSave',JSON.stringify(state)); }
function loadLocal(){ 
...
function loadFromCode(){
    ...
    Object.assign(state,JSON.parse(atob(data)));
    renderAll();
...
adminBtn.onclick = () => {
    ...
    if(pass === "I867") { adminPanel.style.display = "block"; adminBtn.style.display = "none"; }
    else alert("Incorrect password!");
};

function toggleAdminPanel(){ adminPanel.style.display = adminPanel.style.display==="none"?"block":"none"; }

function adminInsertMoney(){
    const amt = parseFloat(document.getElementById('adminMoneyInput').value);
    if(!isNaN(amt)&&amt>0){ state.hairs+=amt; saveLocal(); renderAll(); }
}
function adminSetInterval(){
    const intervalSec=parseFloat(document.getElementById('adminIntervalInput').value);
    if(!isNaN(intervalSec)&&intervalSec>0){
        clearInterval(autoTickInterval);
        autoTickInterval=setInterval(autoTick, intervalSec*1000);
    }
}
function adminBuyUpgrades(){
    const num=parseInt(document.getElementById('adminBuyInput').value);
    if(!isNaN(num)&&num>0){
        state.upgrades.forEach(u=>{ u.level+=num; u.cost+=u.scale*num; });
        saveLocal(); renderAll();
    }
}
function adminYameen(){
    document.body.style.fontFamily="Impact, Charcoal, sans-serif";
    document.querySelectorAll('#game *').forEach(el=>{
        if(el.tagName!=='INPUT' && el.tagName!=='BUTTON') el.textContent="Yameen";
    });
}
